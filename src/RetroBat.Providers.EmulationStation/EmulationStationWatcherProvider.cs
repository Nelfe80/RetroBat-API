...
public class EmulationStationWatcherProvider : IProvider
{
    ...
    private async Task ProcessEventAsync(long sequence, string eventName, string[] args)
    {
        ...
        else if (eventName == "game-start")
        {
            ...
            if (args.Length > 0)
            {
                ...
                await PublishRawFrontendEventAsync(
                    ...
                    eventName,
                    args,
                    BuildFrontendGameSelection(_context.Ui.Running, sysId, longName),
                    sequence);
            }
        }
        else if (eventName == "game-end")
        {
            ...
            await PublishRawFrontendEventAsync(
                ...
                eventName,
                args,
                BuildFrontendGameSelection(finishedGame, finishedGame?.SystemId ?? _context.Ui.SelectedSystem?.Name ?? string.Empty),
                sequence);
            _context.Ui.State = "browsing";
    ...
    private static FrontendGameSelectionSnapshot BuildFrontendGameSelection(
        GameReference? game,
        string systemIdFallback,
        string longName = "")
    {
        var details = game?.Details;
        return new FrontendGameSelectionSnapshot
        {
            SystemId = FirstNonEmpty(game?.SystemId, systemIdFallback),
            ...
            Name = FirstNonEmpty(details?.Name, game?.GameName, longName),
    ...
    private sealed class FrontendGameSelectionSnapshot
    {
