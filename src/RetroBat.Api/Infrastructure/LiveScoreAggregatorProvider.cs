...
public sealed class LiveScoreAggregatorProvider : IProvider
{
    ...
    private static readonly Regex CurrentPlayerRegex = new(
        @"\b(?:current|active)\s+player\b|\bplayer\s+(?:turn|state)\b|\bturn\s+player\b",
        RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
    ...
    // High-score / leaderboard reference values that are NOT the live current score. These must
    // be kept out of the live score sum (e.g. multi-game carts, or games exposing their record
    // table in RAM) so they don't pollute it.
    private static readonly Regex ReferenceScoreRegex = new(
        @"\b(?:hi[-\s]?scores?|high[-\s]?scores?|top\s+scores?|best\s+scores?|leaderboard|first\s+place|1st\s+place|personal\s+best|world\s+record|all[-\s]?time)\b",
        RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
    ...
    private void TryUpdateCurrentPlayer(EventEnvelope envelope)
    {
        ...
        if (!IsCurrentPlayerSignal(action, channel, description))
        {
            return;
        }
    ...
    private void TryHandleMemoryScore(EventEnvelope envelope)
    {
        ...
        var description = FirstNonEmpty(
            ...
            ReadString(payload, "sourceCategory"),
            ReadString(payload, "SourceCategory"));
        // "Top Score", "Leaderboard First Place Score"… are record references, not the live
        // current score. Do not aggregate them, otherwise they inflate the live score sum
        // (e.g. Tetris & Dr Mario showed current + top + leaderboard added together).
        if (ReferenceScoreRegex.IsMatch(description))
        {
            return;
        }
        var source = NormalizeMemorySource(ReadString(payload, "Source"));
    ...
    private static bool IsCurrentPlayerSignal(string? action, string channel, string description)
    {
        return IsCurrentPlayerText(action) ||
            IsCurrentPlayerText(channel) ||
            IsCurrentPlayerText(description);

        static bool IsCurrentPlayerText(string? value)
        {
            ...
            if (normalized.Equals("PLAYER", StringComparison.OrdinalIgnoreCase) ||
                normalized.Equals("PLAYER_STATE", StringComparison.OrdinalIgnoreCase) ||
                normalized.Equals("CURRENT_PLAYER", StringComparison.OrdinalIgnoreCase) ||
                normalized.Equals("CURRENT_PLAYER_STATE", StringComparison.OrdinalIgnoreCase) ||
                normalized.Equals("ACTIVE_PLAYER", StringComparison.OrdinalIgnoreCase) ||
                normalized.Equals("PLAYER_TURN", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }

            return CurrentPlayerRegex.IsMatch(normalized);
        }
    }
    ...
    {
        // 1) Explicit formula: "Score (value * 655360)".
        var formula = WeightRegex.Match(description);
        if (formula.Success && long.TryParse(formula.Groups["weight"].Value, out var fw) && fw > 0)
        {
            return fw;
        }

        // 2) Numeric place: "1s place", "10,000s place", "100,000,000,000s place".
        var place = Regex.Match(
            description,
            @"(?<![A-Za-z0-9])(?<n>\d[\d,]*)\s*s?\s+place",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        if (place.Success &&
            long.TryParse(place.Groups["n"].Value.Replace(",", string.Empty), NumberStyles.Integer, CultureInfo.InvariantCulture, out var pv) &&
            pv > 0)
        {
            return pv;
        }

        // 3) Spelled-out place: "(Tens)", "(Ten Thousands)", "(Hundred Thousands)", "Millions"…
        return InferWeightFromWords(description);
    }

    // Maps an English place name to its decimal weight. When several places are named for one
    // byte (a BCD pair, e.g. "ten thousands, hundred thousands") the lowest is returned, which
    // is the byte's actual weight since it carries a 0..99 value at that base place.
    private static long InferWeightFromWords(string description)
    {
        // Only a terse, structurally-marked place label counts (parenthesised, after a - / :
        // separator, after "in/score/points", or followed by "place"/"digit"), anchored to the
        // tail. Prose that merely mentions magnitudes (e.g. "…for Tens and Thousands, and 32
        // for Hundreds") must NOT be read as a single-place weight.
        var m = Regex.Match(
            description,
            @"(?:\(|[-:]\s*|\b(?:in|score|points?)\s+)\s*(?<mag>(?:ten|hundred)\s+(?:thousands?|millions?)|thousands?|millions?|hundreds?|tens?|ones?|units?)(?:\s+(?:place|digit)s?)?\s*\)?\s*$",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        if (!m.Success)
        {
            m = Regex.Match(
                description,
                @"\b(?<mag>(?:ten|hundred)\s+(?:thousands?|millions?)|thousands?|millions?|hundreds?|tens?)\s+(?:place|digit)s?\s*$",
                RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
            if (!m.Success) return 1L;
        }

        var mag = m.Groups["mag"].Value.ToLowerInvariant();
        long baseMagnitude;
        if (mag.Contains("million")) baseMagnitude = 1_000_000L;
        else if (mag.Contains("thousand")) baseMagnitude = 1_000L;
        else if (mag.Contains("hundred")) return 100L;
        else if (mag.Contains("ten")) return 10L;
        else return 1L;

        if (mag.StartsWith("ten")) return baseMagnitude * 10L;
        if (mag.StartsWith("hundred")) return baseMagnitude * 100L;
        ...
        out ScorePartTransform transform)
    {
        ...
        // being zeros. Handles both a BCD pair ("XX00") and a single digit per byte ("X00000",
        // "0X0000", … "00000X"). The weight is 10^(zeros after the last X). Bounded by non-word
        // chars and required length >= 2 so a stray 'x' inside a word is never taken as a mask.
        var match = Regex.Match(
            description,
            @"(?<![A-Za-z0-9])(?<mask>[0X,]*X[0X,]*)(?![A-Za-z0-9])",
            RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        if (!match.Success)
        {
            return false;
        ...
        // Masks may carry thousands separators (e.g. "00X,X00"); strip them before counting.
        var mask = match.Groups["mask"].Value.ToUpperInvariant().Replace(",", string.Empty);
        if (mask.Length < 2 || !mask.Contains('X'))
        {
            return false;
        }
