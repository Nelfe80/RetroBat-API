...
public class WebSocketConnectionManager
{
    ...
    private string? _retainedRetroAchievementsCatalogJson;
    ...
    public async Task AddSocketAsync(WebSocket socket, string? stream)
    {
        ...
        IReadOnlyList<string> retainedMessages;
        await _lock.WaitAsync();
        try
        {
            _sockets.Add(new WebSocketSubscription(socket, normalizedStream));
            retainedMessages = GetRetainedMessagesForStream(normalizedStream);
        }
        finally
        {
            _lock.Release();
    ...
    private static bool IsAdaptiveVisualKey(string key)
    {
        return key.StartsWith("frontend.selection", StringComparison.OrdinalIgnoreCase) ||
            ...
            string.Equals(key, "screen.snapshot", StringComparison.OrdinalIgnoreCase);
    }

    private IReadOnlyList<string> GetRetainedMessagesForStream(string stream)
    {
        ...
        {
            return One(_retainedScoreJson);
        }

        if (string.Equals(stream, "timer", StringComparison.OrdinalIgnoreCase))
        {
            return One(_retainedTimerJson);
        }

        if (string.Equals(stream, "retroachievements", StringComparison.OrdinalIgnoreCase))
        {
            return new[] { _retainedRetroAchievementsCatalogJson, _retainedRetroAchievementsSessionJson }
    ...
    private void RetainMessage<T>(T message, string json)
    {
        ...
        {
            _retainedRetroAchievementsSessionJson = json;
        }

        if (message is EventEnvelope raEndedEnvelope &&
            string.Equals(raEndedEnvelope.Type, "retroachievements.session.ended", StringComparison.OrdinalIgnoreCase))
        {
            _retainedRetroAchievementsSessionJson = null;
        }

        if (message is EventEnvelope raCatalogEnvelope &&
            string.Equals(raCatalogEnvelope.Type, "retroachievements.catalog.updated", StringComparison.OrdinalIgnoreCase))
        {
            _retainedRetroAchievementsCatalogJson = json;
        }
    }
