...
public sealed class RomsMediaCanonicalMigrationHostedService : IHostedService
{
    ...
    {
        return Path.Combine(RetroBatPaths.RuntimeLogRoot, "roms-media-canonical-migration.snapshot.json");
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var logPath = Path.Combine(RetroBatPaths.RuntimeLogRoot, "roms-media-canonical-migration.jsonl");
