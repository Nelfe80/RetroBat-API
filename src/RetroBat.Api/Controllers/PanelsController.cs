...
public class PanelsController : ControllerBase
{
    ...
    /// <summary>
    /// Deploys the per-game FBNeo RetroArch remaps (one rom, or the whole set).
    /// User-modified files are kept.
    /// </summary>
    /// <remarks>
    /// The remap is generated by composing the game's dynpanel placement, the cabinet
    /// cartography, and the RetroPad ids the CORE declares for that game - measured by
    /// tools/libretro-probe and stored in the dynpanel. A game with no measurement is
    /// left alone rather than mapped by guesswork. Chunkable with offset/limit.
    /// </remarks>
